...
<div class="main-content">
    {{-- BREADCRUMB --}}
    <x-breadcrumb :heading="__('Meja')" :sub-heading="__('Daftar Meja')"></x-breadcrumb>
    {{-- END BREADCRUMB --}}

    <div class="row g-3 align-items-end">
        <div class="col-12 col-md-6 col-lg-4">
            <label for="search-input" class="form-label mb-2">Cari Meja</label>
        ...
        <div class="card-body">
            <div class="table-responsive white-space-nowrap">
                <table class="table align-middle">
                    ...
                    <tbody>
                    @forelse($this->tables as $key => $value)
                        <tr>
                            <td>{{ ($this->tables->currentPage() - 1) * $this->tables->perPage() + $key + 1}}</td>
                            <td>{{ $value->name }}</td>
                            <td><span
                                    class="badge {{ $value->status ? "bg-grd-success" : "bg-grd-danger" }}">{{ $value->status ? "Tersedia" : "Tidak Tersedia" }}</span>
                            </td>
    ...
    {{-- Modal Form --}}
    <livewire:pages::admin-panels.tables.form/>
    @endisland
</div>
...
<script>
    $(document).ready(function () {
        ...
        $('#statusTable').on('change', function () {
            var data = $('#statusTable').select2("val");
